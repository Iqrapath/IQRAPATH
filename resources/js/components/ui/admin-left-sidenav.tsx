...
interface AdminLeftSidenavProps extends React.HTMLAttributes<HTMLDivElement> {}

type IconType = LucideIcon | any; // Allow any type for icons to support HugeIcons

interface NavItem {
    title: string;
    href: string;
    icon: IconType;
    iconType?: 'lucide' | 'hugeicons' | 'custom';
    divider?: boolean;
...
const IconRenderer = ({ icon, size = 20, type = 'lucide' }: { icon: IconType; size?: number; type?: 'lucide' | 'hugeicons' | 'custom' }) => {
    if (type === 'hugeicons') {
...
export function AdminLeftSidenav({ className, ...props }: AdminLeftSidenavProps) {
    const { url } = usePage();
